...
function get(booklId) {
  return storageService.get(BOOK_KEY, booklId)
    .then(book => {
      return _setNextPrevBookId(book)
    })
}

function remove(booklId) {
  return storageService.remove(BOOK_KEY, booklId)
}
...
function _createBooks() {
  ...
  if (!books || !books.length) {
    ...
    // books.push(_createBook('a-book', 30))
    // books.push(_createBook('b-book', 60))
    // books.push(_createBook('c-book', 10))
    // books.push(_createBook('d-book', 15))
